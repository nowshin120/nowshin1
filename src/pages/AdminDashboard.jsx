...
export default function AdminDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    let nextBannerText = '';
    let nextBannerActive = true;
    let legacyDesktop = '';
    let legacyMobile = '';
    let parsedDesktopSlides = [];
    let parsedMobileSlides = [];

    (settingsRes.data ?? []).forEach((item) => {
      if (item.key === 'banner_text') nextBannerText = item.value ?? '';
      if (item.key === 'banner_active') nextBannerActive = item.value !== 'false';
      if (item.key === 'banner_image_url') legacyDesktop = item.value ?? '';
      if (item.key === 'banner_image_url_mobile') legacyMobile = item.value ?? '';
      if (item.key === 'banner_slides_desktop') parsedDesktopSlides = parseSlides(item.value);
      if (item.key === 'banner_slides_mobile') parsedMobileSlides = parseSlides(item.value);
    });

    if (!parsedDesktopSlides.length && legacyDesktop) {
      parsedDesktopSlides = [{ id: 'desktop-1', imageUrl: legacyDesktop }];
    }

    if (!parsedMobileSlides.length && legacyMobile) {
      parsedMobileSlides = [{ id: 'mobile-1', imageUrl: legacyMobile }];
    }

    if (!parsedMobileSlides.length && parsedDesktopSlides.length) {
      parsedMobileSlides = parsedDesktopSlides;
    }

    setBannerText(nextBannerText);
    setBannerActive(nextBannerActive);
    setDesktopBannerSlots(normalizeBannerSlots(parsedDesktopSlides));
    setMobileBannerSlots(normalizeBannerSlots(parsedMobileSlides));
  ...
  const updateBannerSlot = (setter, index, patch) => {
    setter((current) =>
  ...
  const handleBannerFileChange = (setter, index) => (event) => {
    ...
    updateBannerSlot(setter, index, {
      file,
      preview: URL.createObjectURL(file),
    });
  };

  const handleBannerSlotClear = (setter, index) => () => {
    updateBannerSlot(setter, index, {
      file: null,
      imageUrl: '',
      preview: '',
  ...
  const uploadBannerCollection = async (slots) => {
    const resolved = [];

    for (const slot of slots) {
      if (slot.file) {
        const imageUrl = await uploadToCloudinary(slot.file);
        resolved.push({
          ...slot,
          imageUrl,
          preview: imageUrl,
  ...
  const handleEditProduct = (product) => {
    ...
    setFormData({
      ...
      category_slug: product.category_slug ?? EMPTY_FORM.category_slug,
  ...
  const handleBannerSave = async () => {
    ...
    try {
      const nextDesktopSlots = await uploadBannerCollection(desktopBannerSlots);
      const nextMobileSlots = await uploadBannerCollection(mobileBannerSlots);

      const desktopPayload = buildBannerPayload(nextDesktopSlots, 'desktop');
      const mobilePayload = buildBannerPayload(nextMobileSlots, 'mobile');
      ...
      const { error } = await supabase.from('site_settings').upsert(
        [
          ...
          {
            key: 'banner_slides_desktop',
            value: JSON.stringify(desktopPayload),
            updated_at: new Date().toISOString(),
          },
          {
            key: 'banner_slides_mobile',
            value: JSON.stringify(mobilePayload),
            updated_at: new Date().toISOString(),
          },
          {
            key: 'banner_image_url',
            value: desktopPayload[0]?.imageUrl || '',
            updated_at: new Date().toISOString(),
          },
          {
            key: 'banner_image_url_mobile',
            value: mobilePayload[0]?.imageUrl || desktopPayload[0]?.imageUrl || '',
            updated_at: new Date().toISOString(),
          },
        ],
        { onConflict: 'key' }
      ...
      setDesktopBannerSlots(nextDesktopSlots);
      setMobileBannerSlots(nextMobileSlots);
      showToast('success', 'মাল্টি-ব্যানার আপডেট হয়েছে।');
    } catch (error) {
  ...
  const navItems = [
    ...
    { id: 'banner', label: 'মাল্টি ব্যানার', icon: Megaphone },
          ...
          {tab === 'overview' ? (
            <div className="mx-auto max-w-5xl space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">ড্যাশবোর্ড সারাংশ</p>
              </div>

              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                {[
                  { label: 'প্রোডাক্ট', value: products.length, icon: Package, color: 'bg-blue-500' },
                  { label: 'অ্যাডমিন', value: userCount, icon: Users, color: 'bg-violet-500' },
                  { label: 'মেসেজ', value: messageCount, icon: MessageCircle, color: 'bg-emerald-500' },
                  {
                    label: 'ব্যানার',
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-gray-200 bg-white py-3 pl-10 pr-4 text-sm outline-none transition focus:border-blue-400"
                  />
                </div>
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  <div className="divide-y divide-gray-100">
                    {filteredProducts.map((product) => {
                      const category = findCategoryBySlug(product.category_slug);
                        ...
                        <div key={product.id} className="flex items-center gap-4 px-5 py-4">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-2 flex items-center gap-2">
                              {category ? (
                                <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${category.badgeClass}`}>
                                  {getCategoryLabel(category, 'bn')}
                                </span>
                              ) : null}
                              <span className="text-xs text-gray-400">
                ...
                <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                      নতুন category, image আর delivery charge সহ প্রোডাক্ট সেভ করুন।
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      ...
                      <input
                        ...
                        placeholder="যেমন: স্মার্ট ওয়াচ / আইফোন / কিডস ড্রেস"
                ...
                <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900">
                      ...
                      মাল্টি ব্যানার ম্যানেজমেন্ট
                    </h2>
                    <p className="mt-1 text-sm text-gray-400">
                      ডেস্কটপ আর মোবাইলের জন্য ৪টি পর্যন্ত স্লাইড দিন। সাইটে ৩-৪ সেকেন্ড পর পর অটো-রোটেট হবে।
                    </p>
                  </div>

                  <div className="rounded-2xl bg-amber-50 px-4 py-2 text-xs font-medium text-amber-700">
                    Dark overlay সরানো হয়েছে, তাই ব্যানার light ও clean দেখাবে।
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="যেমন: নতুন কালেকশন, ফিচারড অফার, স্মার্ট গ্যাজেট..."
                  ...
                  <div className="grid gap-5 lg:grid-cols-2">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h3 className="text-sm font-bold text-blue-700">ডেস্কটপ ব্যানার স্লাইড</h3>
                        <span className="rounded-full bg-blue-50 px-3 py-1 text-[11px] font-semibold text-blue-600">
                          1440×420 or wide ratio
                        </span>
                      </div>

                      <div className="grid gap-4 sm:grid-cols-2">
                        {desktopBannerSlots.map((slot, index) => (
                          <BannerSlotCard
                            key={slot.id}
                            title={`ডেস্কটপ স্লাইড ${index + 1}`}
                            hint="Hero desktop banner"
                            accent="blue"
                            slot={slot}
                            onFileChange={handleBannerFileChange(setDesktopBannerSlots, index)}
                            onClear={handleBannerSlotClear(setDesktopBannerSlots, index)}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h3 className="text-sm font-bold text-amber-700">মোবাইল ব্যানার স্লাইড</h3>
                        <span className="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-600">
                          750×450 or 4:3 ratio
                        </span>
                      </div>

                      <div className="grid gap-4 sm:grid-cols-2">
                        {mobileBannerSlots.map((slot, index) => (
                          <BannerSlotCard
                            key={slot.id}
                            title={`মোবাইল স্লাইড ${index + 1}`}
                            hint="Hero mobile banner"
                            accent="amber"
                            slot={slot}
                            onFileChange={handleBannerFileChange(setMobileBannerSlots, index)}
                            onClear={handleBannerSlotClear(setMobileBannerSlots, index)}
                          />
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    মোবাইল ব্যানার ফাঁকা থাকলে desktop banner fallback হিসেবে দেখাবে।
                  </div>
